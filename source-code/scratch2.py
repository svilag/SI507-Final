...
def check_cache(url:str) -> str:
    """ checks if a url is already in the cache. if not, adds it.
        ...
        returns:
            cache_entry(str): html content from the cache
    """
    try:
        stored_cache = read_json('./cache/cache.json')
        logger.info("Checking chache for %s", url)
    except: # pylint: disable=W0702 # disable pylint bare except message
        # https://stackoverflow.com/questions/53408630/catching-all-exceptions-without-pylint-error
        stored_cache = CACHE
        logger.info("Cache empty")
    if url not in stored_cache:
        CACHE[url] = get_content(url)
        logger.info("Added %s to cache", url)
...
def get_links(url: str) -> namedtuple:
    """ parses a url that contains html content with links to scores and recaps
        pages for each competition. Returns a list of named tuples for each competition.

    params:
        url(str): link to html content

    returns:
        competitions_data(list): list of named tuples Competition(comp_name, scores_url, recap_url)
    """
...
#         competition. Returns a list of competitions.
...
#                 scores = "No scores"
#                 recaps = "No recaps"
#             # get_scores here? # TODO
#             if scores != "No scores":
#                 scores_by_group = get_scores(scores) # dict {group:score}
#                 groups = [group for group in scores_by_group] # get list of group names
#             # create Competition class object # assign groups and scores in # TODO assign groups & scores
#                 competition = Competition(comp_name, date, scores, recaps, groups, scores_by_group)
#             # competition = namedtuple(competition, [comp_name, date, scores, recaps, groups=[], scores_by_group={}])
#             # Comp = namedtuple('Competition', ['name', 'date', 'scores', 'groups','scores_by_group']) # define named tuple of comp
#             # competition = Comp(competition, [comp_name, date, scores, recaps, groups, scores_by_group]) # create named tuple for comp
#                 competitions.append(competition) # TODO figure out organization to get groups into comp object


#     return competitions

# def get_scores(url: str) -> dict:
#     """ Parses a page with scores from a competition. Creates Group class objects and writes them to
#         groups.json.
...
#         returns:
#             scores(dict): {group: score}
#     """
#     # TODO DEBUG DOES NOT GRAB SCORES? RETURNS NONE?? not even empty dict???
#     scores= {} # {group: score, ...}
#     groups_to_write = []
#     check_cache(url) # check cache for url content # add to cache if not in
#     soupy = stew(url, cache=True) # get content as BS object
#     scores_div = soupy.find_all('div', attrs={'class': 'table-responsive'}) # list of div elements # len = 1
...
#             # class_groups = [] # groups in this class level # TODO something with this?
...
#             if "percussion" in url.lower() or "perc" in url.lower(): # assign group type
#                 group_type = 'percussion'
#             elif "color-guard" in url.lower():
#                 group_type = 'color_guard'
#             elif "winds" in url.lower():
#                 group_type = 'winds'

#                 group = Group(group_type, group_name, class_level, location) # create Group class object
#             # class_groups.append(group) # add group to list for that class_lvl

#                 group_json = { # TODO move as method to class
...
#                 # check if group in groups file, add to list to write if not
#                 if group_json.get("name") not in read_json("./data/groups.json"):
#                     groups_to_write.append(group_json)
#                 write_json("./data/groups.json", groups_to_write, add=True)
...
if __name__ == '__main__':
    ...
    # scorest = get_scores(TEST_SCORES)
    # print(scorest)
